...
export const metadata: Metadata = {
  ...
  operationId: 'deactivateAnAlias',
};

export const tool: Tool = {
  name: 'deactivate_alias_v1',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nThis endpoint lets you update the status of an alias to in active.\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/v1_deactivate_alias_response',\n  $defs: {\n    v1_deactivate_alias_response: {\n      type: 'object',\n      properties: {\n        data: {\n          type: 'object',\n          properties: {\n            created_at: {\n              type: 'string'\n            },\n            email: {\n              type: 'string'\n            },\n            is_active: {\n              type: 'boolean'\n            },\n            note: {\n              type: 'string'\n            },\n            total_blocked: {\n              type: 'integer'\n            },\n            total_forwarded: {\n              type: 'integer'\n            },\n            updated_at: {\n              type: 'string'\n            }\n          }\n        }\n      }\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      email: {
        type: 'string',
        description: 'The email you want to deactivate',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: Hidemail, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.v1.deactivateAlias(body)));
  } catch (error) {
    if (isJqError(error)) {
