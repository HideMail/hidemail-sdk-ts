...
import { ClientOptions } from 'hidemail';

const prompt = `Runs JavaScript code to interact with the Hidemail API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    HIDEMAIL_API_KEY: requireValue(
      readEnv('HIDEMAIL_API_KEY') ?? client.apiKey,
      'set HIDEMAIL_API_KEY environment variable or provide apiKey client option',
    ),
    HIDEMAIL_BASE_URL: readEnv('HIDEMAIL_BASE_URL') ?? client.baseURL ?? undefined,
  };
